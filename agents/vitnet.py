...
from graphs.models.Res_Unet import ResUNet
from graphs.models.Vitnet import Vitnet
from datasets.resunet import ResUnetDataLoader
from datasets.transunet import TransUnetDataLoader
from torchvision.utils import save_image
from utils.metrix_psy import Metrix
from graphs.models.TransUnet import TransUNet
from datasets.vitnet import VitnetDataLoader
cudnn.benchmark = True
cudnn.deterministic = True

class VitnetAgent(BaseAgent):

    def __init__(self, config):
        super(VitnetAgent, self).__init__(config)

        # 定义模型
        self.model = Vitnet()
        # 定义 data_loader
        self.data_loader = VitnetDataLoader(config=config)
        # 定义 loss
    ...
    def save_checkpoint(self):
        ...
        state = {'model': self.model.state_dict(), 'optimizer': self.optimizer.state_dict(),
                 'epoch': self.config.max_epoch}
        model_name = self.config.model_name
        new_save = os.path.join(self.config.save_checkpoint_path, model_name + '_{}.pth'.format(self.current_epoch))
    ...
    def train_one_epoch(self):
        ...
        for batch_idx, (img, label) in enumerate(self.data_loader.train_loader):
            ...
            label = torchvision.transforms.Resize((224, 224))(label)
