...
class ResUNet(nn.Module):
    def __init__(self, input=3,output=1,decoder='unet'):
        super(ResUNet, self).__init__()

        # U-Net的下采样部分"unetplusplus"
        self.resnet50 = resnet50(pretrained=True)
        if decoder == 'unet' :
            self.decoderNet = UnetDecoder(f0_dim=3, f1_dim=64, f2_dim=256, f3_dim=512, f4_dim=1024, f5_dim=2048,numclass=output)
        else:
            print("decoder is not exits!")
        # Transformer编码器部分
        # 定义Transformer的编码器层
    ...
    def forward(self, x):
        ...
        #print(f5.shape, f4.shape, f3.shape, f2.shape, f1.shape,x.shape)
        out = self.decoderNet(x, f1, f2, f3, f4, f5)
        return out
